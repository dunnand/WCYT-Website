...
(function () {
  ...
  function renderHero() {
    ...
    const history    = songHistory.slice(0, 10);
    const history2   = songHistory2.slice(0, 10);
            ...
            ${(history.length || bsiRecent1.length) ? `
              <div class="wcyt-hero-recent">
                ...
                <ul class="wcyt-hero-recent-list">
                  ${(() => { const seen = new Set(history.map(s => artCacheKey(s.artist, s.title))); const merged = [...history, ...bsiRecent1.filter(s => !seen.has(artCacheKey(s.artist, s.title)))].slice(0, 10); return merged; })().map(s => `
                    <li>
          ...
          ` : `
            ...
            ${(history2.length || bsiRecent2.length) ? `
              <div class="wcyt-hero-recent">
                <div class="wcyt-hero-recent-label">RECENTLY PLAYED</div>
                <ul class="wcyt-hero-recent-list">
                  ${(() => { const seen = new Set(history2.map(s => artCacheKey(s.artist, s.title))); const merged = [...history2, ...bsiRecent2.filter(s => !seen.has(artCacheKey(s.artist, s.title)))].slice(0, 10); return merged; })().map(s => `
                    <li>
                      ${artImg(resolvedArt(s), 96, 'wcyt-hero-recent-art')}
                      <span class="wcyt-hero-recent-track">
                        <span class="wcyt-hero-recent-artist">${esc(s.artist)}</span>
